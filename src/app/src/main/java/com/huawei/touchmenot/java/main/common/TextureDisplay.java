...
public class TextureDisplay {
     ...
     */
    public void init() {
        int[] textures = new int[Constants.INIT_ONE];
        GLES20.glGenTextures(Constants.INIT_ONE, textures, Constants.INIT_ZERO);
        mExternalTextureId = textures[Constants.INIT_ZERO];
    ...
    public void onDrawFrame(ARFrame frame) {
        ShaderUtil.checkGlError(TAG, String.valueOf(R.string.On_draw_frame_start));
        if (frame == null) {
            return;
        }
        ...
        ShaderUtil.checkGlError(TAG, String.valueOf(R.string.On_draw_frame_end_));
    }
    ...
    private static int createGlProgram() {
        ...
        if (program != Constants.INIT_ZERO) {
            ...
            int[] linkStatus = new int[Constants.INIT_ONE];
    ...
    private static int loadShader(int shaderType, String source) {
        ...
        if (Constants.INIT_ZERO != shader) {
            ...
            int[] compiled = new int[Constants.INIT_ONE];
    ...
    private void initBuffers() {
        ...
        ByteBuffer byteBufferForVer = ByteBuffer.allocateDirect(INIT_32);
        ...
        ByteBuffer byteBufferForTex = ByteBuffer.allocateDirect(INIT_32);
        ...
        ByteBuffer byteBufferForTransformedTex = ByteBuffer.allocateDirect(INIT_32);
